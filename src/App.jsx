...
export default function App() {
 ...
 // Auto-update: register SW and listen for new versions
 useEffect(() => {
  if (!('serviceWorker' in navigator)) return;
  navigator.serviceWorker.register('/sw.js').then(reg => {
   // Check for updates every 60 seconds while app is open
   setInterval(() => reg.update(), 60000);
   // When a new version is found and installed, activate it
   reg.addEventListener('updatefound', () => {
    const newWorker = reg.installing;
    if (!newWorker) return;
    newWorker.addEventListener('statechange', () => {
     if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
      // New version available — tell it to take control
      newWorker.postMessage('SKIP_WAITING');
     }
    });
   });
  }).catch(() => {});
  // When the new service worker takes control, reload the page once
navigator.serviceWorker.addEventListener('controllerchange', () => {
  if (sessionStorage.getItem('sw_refreshing')) return;
 ...
 {/* ❓ info button */}
 <div onClick={(e)=>{e.stopPropagation();setShowCarInfo(!showCarInfo);}} style={{position:"absolute",top:8,right:8,width:20,height:20,borderRadius:"50%",background:"rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:11,color:"rgba(255,255,255,0.4)",zIndex:10,fontWeight:700}}>?</div>
 {showCarInfo && (
 ...
 {card.data.link && <button onClick={()=>window.open(card.data.link,"_blank")} style={{...S.btn("rgba(143,188,74,0.2)","#8FBC4A",8,"6px 12px"),fontSize:11,border:"1px solid rgba(143,188,74,0.3)"}}>🔗 Más info</button>}
